...
public class PooledSMPPSession<T extends SMPPSession> implements AutoCloseable {
    ...
    private final GenericObjectPool<ThrottledSMPPSession> pool;
    ...
    public PooledSMPPSession(final String host, final int port, final boolean ssl, final String systemId,
                             ...
                             final int maxTotal,
                             final int minIdle,
                             final int maxIdle, final double messageRate, final int maxConcurrentRequests, final int pduProcessorDegree) {
        this.pool = createObjectPool(host, port, ssl, systemId, password, systemType, messageReceiverListener,
    ...
    private GenericObjectPool<ThrottledSMPPSession> createObjectPool(final String host, final int port, final boolean ssl,
        ...
        final GenericObjectPoolConfig<ThrottledSMPPSession> config = new GenericObjectPoolConfig<>();
...
//    pool.setAbandonedConfig(abandonedConfig);
    ...
    public ThrottledSMPPSession borrowObject() throws Exception {
        ...
        log.debug("type {}, {}", session.getClass().getTypeName(),session.getClass().getName());
    ...
    public void returnObject(final ThrottledSMPPSession session) {
